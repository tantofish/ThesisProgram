...
void Model_3DS::Load(char *name, int modelLabel)
{
	mdFlag = modelLabel;
...
void Model_3DS::Draw()
{
	...
	glPushMatrix();
		...
		glScalef(scale*0.9f, scale*0.9f, scale*0.9f);

		glTranslatef(0.f, 60.f, -114.f);
...
void Model_3DS::EditChunkProcessor(long length, long findex)
{
	...
	*/
	...
	/*
	printf("MAX(%f,%f,%f), Min((%f,%f,%f))", max_X, max_Y, max_Z, min_X, min_Y, min_Z);
...
void Model_3DS::VertexListChunkProcessor(long length, long findex, int objindex)
{
	...
	float OffsetX = -184.31f;
	float OffsetY = 3364.19f;
	float OffsetZ = 591.16f;
	float scale = 1.f;

	switch(mdFlag){
	case 0:
		OffsetX = -184.31f;	OffsetY = 3364.19f;	OffsetZ = 591.16f;	scale = 181.f/400.f;
		break;
	case 1:
		OffsetX = 25.44f;	OffsetY = -31.46f;	OffsetZ = 0.f;	scale = 181.f/275;
		break;
	default:
		OffsetX = 0.f;	OffsetY = 0.f;	OffsetZ = 0.f;	scale = 1.f;
		break;
	}
	...
	for (int i = 0; i < numVerts * 3; i+=3)
	{
		...
		(Objects[objindex].Vertexes[i+0] -= OffsetX) *= scale;
		(Objects[objindex].Vertexes[i+1] -= OffsetY) *= scale;
		(Objects[objindex].Vertexes[i+2] -= OffsetZ) *= scale;

		/*
		if(Objects[objindex].Vertexes[i+0]>max_X)	max_X = Objects[objindex].Vertexes[i+0];
		if(Objects[objindex].Vertexes[i+1]>max_Y)	max_Y = Objects[objindex].Vertexes[i+1];
		if(Objects[objindex].Vertexes[i+2]>max_Z)	max_Z = Objects[objindex].Vertexes[i+2];
		if(Objects[objindex].Vertexes[i+0]<min_X)	min_X = Objects[objindex].Vertexes[i+0];
		if(Objects[objindex].Vertexes[i+1]<min_Y)	min_Y = Objects[objindex].Vertexes[i+1];
		if(Objects[objindex].Vertexes[i+2]<min_Z)	min_Z = Objects[objindex].Vertexes[i+2];
		*/
	}
